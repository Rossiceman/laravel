    ...
    function delStore(id, title) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "/admin/product/layer1/delete",
                    type: "post",
                    data: {
                        id: id,
                        _token: "{{ csrf_token() }}"
            ...
            <div class="card-header">
                ...
                <table class="table border border-dark">
                    <tr class="table-warning">
                        <td class="col-8 text-center border border-dark">大分類</td>
                        <td class="col-2 text-center border border-dark">修改</td>
                        <td class="col-2 text-center border border-dark">刪除</td>
                    </tr>
                    @foreach($list as $data)
                    <tr id="row{{ $data->id }}">
                        <td class="text-center border border-dark">{{ $data->layer1_name }}</td>
                        <td class="text-center border border-dark">
                            <a href="edit/{{ $data->id }}" class="btn btn-success">修改</a>
                        </td>
                        <td class="text-center border border-dark">
                            <a href="javascript:delStore('{{ $data->id }}', '{{ $data->layer1_name }}')" class="btn btn-danger">刪除</a>
                        </td>
                    </tr>
                    @endforeach
                </table>
