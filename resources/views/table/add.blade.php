                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-10">
                            ...
                            @endif
                                ...
                                <input type="radio" class="form-check-input border border-dark" name="s{{ $data1->id }}" value="{{ $data2->id }}" onclick="doEvents('{{ $data2->events }}')">
                                @else
                                    <input type="radio" class="form-check-input border border-dark"
                                    name="s{{ $data1->id }}" value="{{ $data2->id }}">
                                @endif
                                @elseif($data2->types == "c")
                                <input type="checkbox" class="form-check-input border border-dark" name="s{{ $data2->id }}" value="{{ $data2->id }}">
                                @elseif($data2->types == "X")
                                <input type="text" class="form-control border border-dark" name="s{{ $data2->id }}">
                                @endif
                                {{ $data2->s2_name }}
                            </div>
                            @endforeach
